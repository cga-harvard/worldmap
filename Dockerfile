...
RUN mkdir -p /usr/src/worldmap

WORKDIR /usr/src/worldmap
...
COPY . /usr/src/worldmap

RUN chmod +x /usr/src/worldmap/tasks.py \
    && chmod +x /usr/src/worldmap/entrypoint.sh

# app-specific requirements
RUN pip install --upgrade --no-cache-dir --src /usr/src -r requirements.txt
RUN pip install --upgrade -e .

ENTRYPOINT ["/usr/src/worldmap/entrypoint.sh"]